...
| `SCENARIO_TEST_PROJECT_ID` | `tools/dev_config.py`: `live_settings`, `legacy_credentials` | Optional; unset/blank becomes `None`. Current prototype tools cannot select a project and reject nonempty values. Shared SDK adapter adoption will provide project selection | Developer or project-specific fixture |
...
| `SCHEMA_CACHE` | `tools/audit_payloads.py` | Cached schema directory; default `<tempdir>/scenario-schema-cache` | Developer |
...
A project ID is **not required for API-key authentication**. For the existing
prototype tools, use keys scoped to the intended project and leave the optional
project variable blank. Project-selection tests may require their own fixture
and skip clearly when it is absent; that is not a global credential requirement.
