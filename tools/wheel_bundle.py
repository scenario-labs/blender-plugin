...
def validate_lock(lock, manifest):
    ...
    for wheel in lock["wheels"]:
        ...
        if not wheel["licenses"]:
            raise ValueError("Wheel has no recorded license notice")
...
def cached_wheel(wheel, cache, *, offline=False):
    ...
    if path.is_file() and not path.is_symlink() and digest(path.read_bytes()) == wheel["sha256"]:
        return path
    if offline:
    ...
    with tempfile.TemporaryDirectory(prefix="wheel-", dir=cache) as directory:
        ...
        if digest(temporary.read_bytes()) != wheel["sha256"]:
...
def wheel_notices(wheel, raw):
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as archive:
        ...
        if len(set(names)) != len(names):
            raise ValueError("Wheel has duplicate archive members")
        for name in names:
            _safe_path(name)
        notices = {}
        for notice in wheel["licenses"]:
            content = archive.read(notice["path"])
...
def prepare_source(source, destination, *, cache=None, offline=False):
    ...
    notices = {}
    for wheel in lock["wheels"]:
        raw = cached_wheel(wheel, cache, offline=offline).read_bytes()
    ...
    with zipfile.ZipFile(candidate) as archive:
        ...
        for wheel in lock["wheels"]:
            for name, content in wheel_notices(
                wheel, archive.read("wheels/" + wheel["filename"])
            ).items():
